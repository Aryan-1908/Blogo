...
with open('config.json','r') as c:
    aryan= json.load(c)["aryan"]

app.config['Upload_Folder']=aryan['upload_location']
...
app.config.update(
    ...
    MAIL_PORT='465',
    MAIL_USE_SSL=True,
    MAIL_USERNAME=aryan['Username'],
    MAIL_PASSWORD=aryan['Password']
    )
mail=Mail(app)
...
#pagination method
    last = math.ceil(len(posted)/int(aryan['no_of_posts']))
    page = request.args.get('page')
    if (not str(page).isnumeric()):
        page = 1
    page = int(page)
    posted = posted[(page-1)*int(aryan['no_of_posts']):(page-1)*int(aryan['no_of_posts'])+ int(aryan['no_of_posts'])]
    if page==1:
        prev = "#"
    ...
    return render_template('index.html', image_url=img1, aryan=aryan, posted=posted, prev=prev, next=next)
...
def upload():
    if('user' in session and session['user']==aryan['nameuser']):
...
def dash():
    ...
    if('user' in session and session['user']==aryan['nameuser']):
        ...
        return render_template('dashboard.html',aryan=aryan,img_url=img1,posted=posted,last_login=last_login)

    if(request.method=='POST'):
        username=request.form.get('username')
        password=request.form.get('password')
        if username==aryan['nameuser'] and password==aryan['passcode']:
            img1= url_for('static', filename='assets/img/admin3.jpg')
            posted = Posts.query.all()
            ...
            return render_template('dashboard.html',aryan=aryan,img_url=img1,posted=posted,last_login=last_login_time)
...
def post_ext(post_slug):
    ...
    return render_template('post.html',aryan=aryan, post=post,image_url=image_url)
...
def contact():
    if(request.method=='POST'):
        ...
        mail.send_message('Message_from_yout_web',
                           ...
                           recipients=['12312@gmail.com', aryan['Username']],
                           body= f'{message}\n\n sent by :{name} \n Mobile: {phone} \n Sender email:{email}'  )

    img4= url_for('static', filename='assets/img/contact.png')
    return render_template('contact.html',image_url=img4, aryan=aryan)
...
def edit(Sno):
    ...
    if ('user') in session and session['user']==aryan['nameuser']:
        ...
        return render_template('edit.html',aryan=aryan,Sno=Sno,posted=posted,img_url=img4)
